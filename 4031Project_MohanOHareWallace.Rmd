...
# Stepwise Regression (change to forward/backward for variety and flow?)
```{r}
...
```
Confirm/deny suspicions

# Lasso to confirm results
```{r}
...
# Conclusion
After examining this census tract data and running our regressions, we have found that housing price changes in later time periods can be predicted fairly well by changes over time in the values of several important variables and their interactions. This indicates that we will likely be able to predict the gentrification in the next few years based on recent changes, and can potentially export this model to tracts outside the city of Atlanta and its surrounding regions.
