...
# Required imports
Importing packages purrr, readr, dplyr, stats, DataCombine, stringr, MASS, car
...
Using previous academic work, large body of parameter were manually selected. An example is shown below:
```{r}
names = names(all_data)
...
dim(all_data)
```
As shown, all_data reduced by x to y
...
Columns NA entries were adjusted
```{r}
...
Similarly, infinity entries were adjustmed
```{r}
...
avplots(full.model)
```
The mutiple regression model carries an adjusted r^2 of .. Given a large number of columns, we will identify significant variables using a stepwise regression.
