...
Given a large number of columns, we will identify significant variables using a stepwise regression.
...
plot(pred_data)
```
It appears that some of our data points are highly correlated with change in median home value, and others are not. However, all correlations do appear linear, so we should not need to include any quadratic or logarithmic terms.
